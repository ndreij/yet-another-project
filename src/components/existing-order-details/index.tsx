...
import styles from './existing-order-details.module.css'
import { useSelector, useWebSocket } from '../../services/hooks';
...
type ExistingOrderDetailsPropsTypes = {
    orderNumberPositionLeft?: boolean
}

export const ExistingOrderDetails: FC<ExistingOrderDetailsPropsTypes> = ({orderNumberPositionLeft}) => {
    ...
    const orderPrice = useCallback(
        () => (
            orderIngredients.length > 0 ? orderIngredients?.map((ingredient) => ingredient.count ? ingredient.price * ingredient.count : ingredient.price )
    ...
    const match1 = useRouteMatch<MatchParams>(`/feed/:orderId`);
    const match2 = useRouteMatch<MatchParams>(`/profile/orders/:orderId`);
    let orderId: string | undefined; 
    if (match1) {
        orderId = match1?.params.orderId
     } else {
        orderId = match2?.params.orderId
     }


    const orderDetails = useMemo(
        () => orders?.find((order) => String(order.number) === orderId)
            || null,
        [orders, orderId],
    );
        ...
        <>
            {orderDetails &&
                    ...
                    <div className={styles.ordercontainer}>
                        <div className={orderNumberPositionLeft === true ? styles.ordernumberleft : styles.ordernumber }>
                            <p className="text text_type_digits-default">#{orderDetails.number}</p>
                        </div>

                        <p className="text text_type_main-medium">{orderDetails.name}</p>
                        <p className={styles.orderstatus}>Выполнен</p>
                        <p className={styles.orderheader}>Состав:</p>
                        ...
                        <div className={styles.orderingredientslist}>
                            ...
                            {orderIngredients.map((ingredient, index) => {
                                return (
                                        ...
                                        <div className={styles.orderitem} key={uuidv4()}>
                        ...
                        <div className={styles.ordertotalcontainer}>
                            <div className={styles.ordertime}>
                                <p className="text text_type_main-default">{readableDate(orderDetails.createdAt)}</p>
                            </div>
